...
"""
    n = krylov(invec...[,Lenv=default_boundary,Renv=default_boundary,r=...,updatefct=makeHpsi,reorth=false,effZero=defzero,alpha=...,beta=...,psisave=...,start=0,cvg=false,numE=...,saveE=...,goal=1E-12,eigvecs=true])

Generates the krylov expansion with a Lanczos method returning the number `n` iterations that were computed before beta values fall below `effZero`; relevant parameters stored in `alpha`, `beta`, and `psisave`; elements of `invec` are provided to the `updatefct` on each iteration (maximum `r`); `reorth` implements an attempt to maintani orthogonality or not; `numE` is the number of eigenvalues to optimize; `saveE` is a vector to store the previous eigenvalues; `goal` is the tolerance to converge the eigenvalues; `eigvecs` toggles whether to compute the eigenvectors (true) or Krylov vectors (false); `start` begins the Lanczos sequence at a particular alpha, beta, and starting vector
"""
function krylov(invec::R...;Lenv::TensType=default_boundary,Renv::TensType=default_boundary,r::intType = prod(w->size(invec[2],w),1:ndims(invec[2])-1),updatefct::Function=makeHpsi,reorth::Bool=false,effZero::Real=defzero,alpha::Array{S,1}=Array{Float64,1}(undef,r),beta::Array{S,1}=Array{Float64,1}(undef,r),psisave::TensType=Array{typeof(invec[1]),1}(undef,r),start::intType=0,cvg::Bool=false,numE::intType=cvg ? 1 : 0,saveE::Array{S,1}=[1000. for w = 1:numE],goal::S=1E-12,eigvecs::Bool=true) where {R <: TensType, S <: Number}
...
#  end
  ...
  while (n < r || r == 0 || start != 0) && n < mindim && betatest
    n += 1
...
"""
    D,U,alpha,beta = lanczos(invec...[,Lenv=default_boundary,Renv=default_boundary,r=...,updatefct=makeHpsi,reorth=false,effZero=defzero,alpha=...,beta=...,psisave=...,start=0,cvg=false,numE=...,saveE=...,goal=1E-12,eigvecs=true])
...
"""
function lanczos(invec::R...;Lenv::TensType=default_boundary,Renv::TensType=default_boundary,r::intType = size(invec[2],1),updatefct::Function=makeHpsi,reorth::Bool=false,start::intType=0,goal::W=1E-12,effZero::Real=defzero,m::intType=1,alpha::Array{S,1}=Array{Float64,1}(undef,r),beta::Array{S,1}=Array{Float64,1}(undef,r),psisave::TensType=Array{typeof(invec[1]),1}(undef,r),cvg::Bool=false,numE::intType=cvg ? 1 : 0,double::Bool=false, saveE::Array{S,1}=Array{Float64,1}(undef,numE),eigvecs::Bool=true) where {W <: Number, R <: TensType, S <: Number}

  retbundlepsi = krylov(invec...,Lenv=Lenv,Renv=Renv,r=r,updatefct=updatefct,reorth=reorth,effZero=effZero,alpha=alpha,beta=beta,psisave=psisave,cvg=cvg,numE=numE,saveE=saveE,start=start,eigvecs=eigvecs,goal=goal)
...
"""
    D,U,alpha,beta = lanczos(psiinvec...[,Lenv=default_boundary,Renv=default_boundary,r=...,updatefct=makeHpsi,reorth=false,effZero=defzero,alpha=...,beta=...,psisave=...,start=0,cvg=false,numE=...,saveE=...,goal=1E-12,eigvecs=true])

For `psi` (an `AbstractArray`), generates the energies contained in `D`and vectors `U` from a krylov expansion with a Lanczos method returning the number of iterations that were computed before beta values fall below `effZero`; relevant parameters stored in `alpha`, `beta`, and `psisave`; elements of `invec` are provided to the `updatefct` on each iteration (maximum `r`); `reorth` implements an attempt to maintani orthogonality or not; `numE` is the number of eigenvalues to optimize; `saveE` is a vector to store the previous eigenvalues; `goal` is the tolerance to converge the eigenvalues; `eigvecs` toggles whether to compute the eigenvectors (true) or Krylov vectors (false); `start` begins the Lanczos sequence at a particular alpha, beta, and starting vector
"""
function lanczos(psi::AbstractArray,invec::AbstractArray...;Lenv::TensType=default_boundary,Renv::TensType=default_boundary,r::intType = prod(w->size(invec[1],w),1:ndims(invec[1])-1),m::intType=1,goal::W=1E-12,updatefct::Function=makeHpsi,reorth::Bool=false,effZero::Real=defzero,alpha::Array{S,1}=Array{Float64,1}(undef,r),beta::Array{S,1}=Array{Float64,1}(undef,r),psisave::TensType=Array{typeof(psi),1}(undef,r),start::intType=0,cvg::Bool=false,double::Bool=false,numE::intType=cvg ? 1 : 0,saveE::Array{S,1}=Array{Float64,1}(undef,numE)) where {S <: Number, W <: Number}
  ...
  D,retpsi = lanczos(tens(psi),changevec...,Lenv=Lenv,Renv=Renv,r=r,m=m,updatefct=updatefct,reorth=reorth,effZero=effZero,alpha=alpha,beta=beta,psisave=newpsisave,start=start,numE=numE,saveE=saveE)
