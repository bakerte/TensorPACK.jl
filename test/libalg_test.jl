...
for g = 1:length(typevec)
  ...
  local C = TENPACK.libmult!('N','N',alpha,A,B,beta,C,200,100,100,200)
  
  local checkC = TENPACK.libmult('N','N',alpha,A,B,200,100,100,200)
...
#  display(C)
  ...
  local checkC = TENPACK.trmm!(B,100,100,A,100,side='L')
...
#  tA = tens(A)
  ...
  local Q,R = TENPACK.libqr!(copy(A),100,200)
  local aQ,aR = TENPACK.libqr(copy(A),100,200)
...
#  tA = tens(A)
  ...
  local Q,R = TENPACK.liblq!(copy(A),100,200)
  local aQ,aR = TENPACK.liblq!(copy(A),100,200)
