...
QNsummary,leftSummary,rightSummary,newQblocksum = TENPACK.makeQNsummaries(Qlabels,[1,2],[3,4],false,test())
...
QNsummary,leftSummary,rightSummary,newQblocksum = TENPACK.makeQNsummaries(A,[1,2],[3,4],false)
...
B,C = TENPACK.convertQnumMat(Qlabels)
testval = A.QnumSum == C
testval &= A.QnumMat == B
fulltest &= testfct(testval,"convertQnumMat(Qlabels)")

B = TENPACK.convertQnumMat(Qlabels,A.QnumSum)
testval &= A.QnumMat == B
fulltest &= testfct(testval,"convertQnumMat(Qlabels,QnumSum)")

println()

B = TENPACK.undefMat(ComplexF64,10,10)
...
B = Qtens(ComplexF64,A)
testval = eltype(mB) <: ComplexF64
fulltest &= testfct(testval,"Qtens(ComplexF64,Qtens))")
...
leftQNs,Lbigtosub,rows,Lindexes,rightQNs,Rbigtosub,columns,Rindexes = TENPACK.QnumList(A,[1,2],[3,4],leftSummary,rightSummary)
...
leftQNs,Lbigtosub,rows,Lindexes,rightQNs,Rbigtosub,columns,Rindexes = TENPACK.QnumList(Lsizes,Rsizes,A.QnumMat,A.QnumSum,[1,2],[3,4],leftSummary,rightSummary)
...
TENPACK.loadindex!(nonzero_sizes,2,[4,4],Lindexes,leftQNs,Lbigtosub,rows)

testval = "Lindexes == [A.ind[i][1] for i = 1:length(A.ind)]"
fulltest &= testfct(testval,"loadindex!(nonzero_sizes,Linds,Lsizes,Lindexes,leftQNs,Lbigtosub,rows)",performancevals)

println()

leftQNs,Lbigtosub,rows = TENPACK.QnumList(Lsizes,A.QnumMat,A.QnumSum,[1,2],leftSummary)
...
Lindexes,Rindexes,leftQNs,rightQNs,rows,columns = TENPACK.makeIndexes([1,2],[4,4],leftQNs,Lbigtosub,rows,[3,4],[4,4],rightQNs,Rbigtosub,columns)
...
fulltest &= testfct(testval,"multi_indexsummary(QnumSum,vec)",performancevals)
...
B = TENPACK.findsizes(A,[1,2])
testval = "B == (2,2,true)"
fulltest &= testfct(testval,"findsizes(Qtens,vec)",performancevals)

println()

B = TENPACK.checkorder!(copy(A),1)
testval = "B == [A.ind[i][1] for i = 1:length(A.ind)]"
fulltest &= testfct(testval,"checkorder!(Qtens,integer)",performancevals)

C = copy(A)
TENPACK.checkorder!(C)
testval = "C.ind == A.ind"
fulltest &= testfct(testval,"checkorder!(Qtens)",performancevals)

println()

B = TENPACK.LRsummary_invQ(leftSummary,A.flux)
testval = "B == (leftSummary,rightSummary)"
fulltest &= testfct(testval,"LRsummary_invQ(QNsummary,flux)",performancevals)
...
testval = 10 == TENPACK.zeropos2ind([1,2,3],(4,5,6),(1,2))
fulltest &= testfct(testval,"zeropos2ind(vec,tup,tup)")
...
fulltest &= testfct(testval,"innerloadpos!(int,int,vec,vec,Matrix)",performancevals)
...
fulltest &= testfct(testval,"reshape!(...)",performancevals)
...
fulltest &= testfct(testval,"getQnum(x,y,QnumMat,QnumSum)",performancevals)

testval = "A.QnumSum[1][1] == getQnum(1,1,A)"
fulltest &= testfct(testval,"getQnum(x,y,Qtens)",performancevals)
...
fulltest &= testfct(testval,"-(Qtens)",performancevals)
